...
export default function LocalAttractions() {
  const [activeCity, setActiveCity] = useState<'amsterdam' | 'paris'>('amsterdam');
  const attractions = activeCity === 'amsterdam' ? amsterdamAttractions : parisAttractions;
  ...
  return (
    <section className="py-24 bg-[var(--cream)]">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        >
          <h2 className="section-title">Discover Local Attractions</h2>
          <p className="section-subtitle">
            Explore the most loved attractions and local experiences.
          </p>
        </motion.div>

        <div className="flex justify-center gap-4 mb-12">
          <button
            onClick={() => setActiveCity('amsterdam')}
            className={`rounded-full px-8 py-3 font-medium transition-all duration-300 ${
              activeCity === 'amsterdam'
                ? 'bg-[var(--forest-green)] text-[var(--cream)] shadow-lg'
                : 'border-2 border-[var(--forest-green)] text-[var(--forest-green)] hover:bg-[var(--forest-green)]/5'
            }`}
          >
            Amsterdam
          </button>
