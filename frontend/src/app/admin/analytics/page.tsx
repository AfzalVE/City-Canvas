...
import { TrendingUp, Users, Eye, Heart, MessageSquare, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function AnalyticsPage() {
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="font-serif text-3xl text-[var(--charcoal)] mb-1">Analytics</h1>
        <p className="text-sm text-[var(--warm-gray)]">Performance overview across all platforms · Last 30 days</p>
      </div>
          ...
          <div key={card.label} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="w-9 h-9 bg-[var(--forest-green)]/10 rounded-lg flex items-center justify-center">
                <card.icon className="w-4 h-4 text-[var(--forest-green)]" />
              </div>
            ...
            <div className="text-2xl font-bold text-[var(--charcoal)] font-serif">{card.value}</div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6">
        <div className="p-5 border-b border-gray-100">
          <h2 className="font-serif text-lg text-[var(--charcoal)]">Platform Performance</h2>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {platformMetrics.map((row) => (
                <tr key={row.platform} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4 font-medium text-[var(--charcoal)]">{row.platform}</td>
                  <td className="px-5 py-4 text-[var(--forest-green)]">{row.followers}</td>
                  ...
                  <td className="px-5 py-4 text-[var(--forest-green)]">{row.posts}</td>
                  <td className="px-5 py-4">
                    {row.engagement !== '—' ? (
                      <span className="font-semibold text-[var(--forest-green)]">{row.engagement}</span>
                    ) : <span className="text-gray-400">—</span>}
                  </td>
                  <td className="px-5 py-4 text-[var(--forest-green)]">{row.reach}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-5 border-b border-gray-100">
          <h2 className="font-serif text-lg text-[var(--charcoal)]">Top Performing Posts</h2>
        </div>
          ...
          {topPosts.map((post, idx) => (
            <div key={post.title} className="px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-[var(--charcoal)]">{post.title}</div>
              ...
              <div className="text-right">
                <div className="text-sm font-semibold text-[var(--forest-green)]">{post.reach.toLocaleString()} reach</div>
